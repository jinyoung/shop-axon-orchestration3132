...
public class InventoryController {
    ...
    public CompletableFuture decreaseInventory(
        ...
        return commandGateway
            .send(decreaseInventoryCommand)
            .thenApply(id -> {
                InventoryAggregate resource = new InventoryAggregate();
                BeanUtils.copyProperties(decreaseInventoryCommand, resource);
    ...
    @RequestMapping(value = "/inventories", method = RequestMethod.POST)
    public CompletableFuture increaseInventory(
        @RequestBody IncreaseInventoryCommand increaseInventoryCommand
    ) throws Exception {
        System.out.println("##### /inventory/increaseInventory  called #####");

        // send command
        return commandGateway
            .send(increaseInventoryCommand)
            .thenApply(id -> {
                InventoryAggregate resource = new InventoryAggregate();
                BeanUtils.copyProperties(increaseInventoryCommand, resource);
